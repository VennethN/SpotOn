...
| `PUBLIC_MAPID_MAP_KEY` | The MAPID MAPS **Map Service key** from the MAPID Dashboard. The style URL is built from it and the light or dark style is picked to match the reader's theme. If empty, an open vector basemap is used (OpenStreetMap data, CARTO cartography) — **mandatory for the final product.** |
