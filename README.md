...
| `OPENROUTER_MODEL` | Optional — one model slug OpenRouter serves, e.g. `anthropic/claude-sonnet-5`, or several separated by commas to replace the whole fallback chain, e.g. `deepseek/deepseek-chat-v3.1:free, anthropic/claude-sonnet-5`. They are tried left to right until one answers, so the first slug is the main model and the rest are its fallbacks. Blank entries are ignored, surrounding quotes are stripped from each slug (the Vercel field and `export` both keep quotes literally, unlike `.env`), and every model named must support function calling. Read at runtime, so changing it on Vercel needs no rebuild. Empty → the free chain in `src/lib/server/llm.ts`. Which models are active can be checked at `GET /api/meta`, where `llm.model` is the main one and `llm.models` the whole chain (the key itself is never included). |
| `MAPID_MAPSERVICES_KEY` | The MAPID MAPS **Map Service key**, from the MAPID Dashboard under **MAP SERVICES**. A different key from `MAPID_API_KEY`, which the basemap service refuses. The style URL is built from it and the light or dark style is picked to match the reader's theme. Read on the server by the `/app` route and handed to the map. If empty, an open vector basemap is used (OpenStreetMap data, CARTO cartography) — **mandatory for the final product.** |
| `PUBLIC_MAPID_MAP_KEY` | The same key under its earlier name. Still read, so an existing deployment keeps working. `MAPID_MAPSERVICES_KEY` wins when both are set. |
