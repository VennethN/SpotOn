...
| `OPENROUTER_MODEL` | Optional — one model slug OpenRouter serves, e.g. `anthropic/claude-sonnet-5`, or several separated by commas to replace the whole fallback chain, e.g. `deepseek/deepseek-chat-v3.1:free, anthropic/claude-sonnet-5`. They are tried left to right until one answers, so the first slug is the main model and the rest are its fallbacks. Blank entries are ignored, and every model named must support function calling. Read at runtime, so changing it on Vercel needs no rebuild. Empty → the free chain in `src/lib/server/llm.ts`. Which models are active can be checked at `GET /api/meta`, where `llm.model` is the main one and `llm.models` the whole chain (the key itself is never included). |
